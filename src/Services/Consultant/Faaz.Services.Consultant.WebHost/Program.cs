...
try
{
    ...
    try
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<ConsultantDbContext>();
        db.Database.Migrate();
    }
    catch (Exception ex)
    {
    ...
    {
        app.UseSwagger();
        app.UseSwaggerUI(opts => opts.SwaggerEndpoint("/swagger/v1/swagger.json", "Faaz Consultant API v1"));
...
{
    Log.Fatal(ex, "Consultant service terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
